...
class MenstruationGaugeCard extends HTMLElement {
  ...
  async _doLogFirstPeriod() {
    try {
      const today = this._isoFromDate(new Date());
      const model = this._buildModel();
      const entityId = model.entityId || this._config?.entity || '';
      const profile = model.stateObj?.attributes?.profile || '';
      const entryId = model.stateObj?.attributes?.entry_id || this._config?.entry_id || '';
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        /* Optionsreihen (15.09.2026, elfte Runde): fest einzeilig mit
           horizontalem Scrollen statt umbrechendem Flex-Wrap, damit
        ...
        /* Symptom-Icon-Kacheln (15.09.2026, aus der iOS/macOS-App uebernommen;
           in der elften Runde von einer Text-Pille mit kleinem Icon links
           auf eine quadratische Kachel mit Icon oben/Text unten umgestellt,
        ...
        /* :has() faellt auf aelteren WebViews (z.B. veraltete Android-System-
           ...
           voll funktionsfaehig, nur ohne farbige Auswahl-Hervorhebung. */
        .sym-opt-label:has(input:checked) { background: ${palette.confirmed}; color: #fff; border-color: ${palette.confirmed}; }
        .sym-opt-label input[type="checkbox"] { position: absolute; opacity: 0; width: 1px; height: 1px; pointer-events: none; accent-color: ${palette.confirmed}; }
